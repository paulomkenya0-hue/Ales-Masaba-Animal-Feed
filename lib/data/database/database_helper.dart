...
class DatabaseHelper {
  ...
  /// v1 -> v2: Ongeza uid (Firebase Auth) na branch_name (jina la duka/tawi
  /// la Cashier) kwenye jedwali la users - zinahitajika kwa usanidi wa
  /// Firebase Auth + Roles (Super Admin / Cashier).
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      await db.execute('ALTER TABLE users ADD COLUMN uid TEXT');
      await db.execute('ALTER TABLE users ADD COLUMN branch_name TEXT');
    }
  }
  ...
  Future<void> _seedDefaultData(Database db) async {
    ...
    // Hii ni akaunti ya "bootstrap" ya ndani (SQLite) pekee - itaunganishwa
