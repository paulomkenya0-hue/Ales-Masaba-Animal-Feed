  ...
  Future<void> _showAddCategoryDialog() async {
    ...
    final name = await showDialog<String>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        title: const Text(SW.newCategory),
        content: TextField(
          controller: ctrl,
          autofocus: true,
    ...
    final newId = await context.read<ProductProvider>().addCategory(name);
    if (mounted && newId != null) {
      setState(() => _categoryId = newId);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Category "$name" imeongezwa')),
      );
    }
  }
