...
class _DashboardScreenState extends State<DashboardScreen> {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isSuperAdmin
            ? SW.dashboard
            : (auth.currentUser?.branchName ?? SW.dashboard)),
        actions: [
          IconButton(
            ...
            child: dash.isLoading
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    // Cashier haoni takwimu za jumla za biashara (stock/ripoti ya fedha)
                    if (!isSuperAdmin)
                      Card(
                        ...
                        child: const Padding(
                          ...
                          child: Row(
                            children: [
                              Icon(Icons.point_of_sale, color: AppColors.primaryGreen),
                              SizedBox(width: 12),
                              Expanded(
                                child: Text(
                                  'Karibu! Tumia kitufe cha "Uzo Mpya" chini kuanza mauzo.',
                                ),
                              ),
                            ],
                          ),
                    ...
                    if (isSuperAdmin) ...[
                      if (dash.lowStockProducts.isNotEmpty || dash.todaySales == 0 || dash.backupOverdue)
                        _buildAlerts(dash),
                    ...
                    _buildQuickActions(context, isSuperAdmin),
  ...
  Widget _buildQuickActions(BuildContext context, bool isSuperAdmin) {
    return Wrap(
      spacing: 12,
      runSpacing: 12,
      children: [
